...
const formatMonthYear = (year, monthIndex) =>
  new Date(year, monthIndex, 1).toLocaleString(undefined, {
    month: 'long',
    year: 'numeric',
  });
...
const getMonthDays = (year, monthIndex) => {
  ...
  return Array.from(
    { length: total },
    (_, i) => formatYYYYMMDD(new Date(year, monthIndex, i + 1))
  );
};
...
// 🔹 Badge Sleep Champ per-user
const unlockSleepBadge = async (sleepByDay, uid) => {
  if (!uid) return;
  try {
    ...
    if (hoursSleptToday >= 7) {
      const badgeDataRaw = await AsyncStorage.getItem(`badges_${uid}`);
      ...
      if (badges.sleepChampDate !== todayKey) {
        ...
        Alert.alert(
          '🏆 Sleep Champ unlocked!',
          "You've slept more than 7 hours!"
        );
      }
    }
  } catch (err) {
...
export default function SleepScreen() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const sessionsKey = `sleep.sessions.v1_${uid}`;
        const scheduleKey = `sleep.schedule.v1_${uid}`;
        const seededKey = `sleep.seeded.v1_${uid}`;

        const [sRaw, schRaw, seeded] = await Promise.all([
        ...
        if ((!parsed || parsed.length === 0) && !seeded) {
          // seed 30 ditë gjumë random (vetëm për demo)
          const now = new Date();
          const seeds = [];
          for (let i = 30; i >= 1; i--) {
            const day = new Date(
            ...
            const startMin = [0, 15, 30, 45][
              Math.floor(Math.random() * 4)
            ];
            const durationMinutes = Math.round(
              6.5 * 60 + Math.random() * (2 * 60)
            );
          ...
          setSessions(seeds);
          await AsyncStorage.setItem(sessionsKey, JSON.stringify(seeds));
          await AsyncStorage.setItem(seededKey, '1');
        }
      } catch (e) {
        console.log('Error loading sleep data', e);
      }
    };
  ...
  useEffect(() => {
    if (!uid) return;
    unlockSleepBadge(sleepByDay, uid);
  }, [sleepByDay, uid]);
  ...
  const saveSchedule = useCallback(
    async (next) => {
      ...
      setSchedule(merged);
      const scheduleKey = `sleep.schedule.v1_${uid}`;
      await AsyncStorage.setItem(scheduleKey, JSON.stringify(merged));
    },
    [schedule, uid]
  );
  ...
  const scheduleAnalysis = useMemo(() => {
    ...
    const bedtimes = recent.map(
      (s) =>
        parseISO(s.startISO).getHours() * 60 +
        parseISO(s.startISO).getMinutes()
    );
    const wakeTimes = recent.map(
      (s) =>
        parseISO(s.endISO).getHours() * 60 +
        parseISO(s.endISO).getMinutes()
    );
    const [schedBed, schedWake] = [schedule.bedtime, schedule.wakeTime].map(
      (t) => parseInt(t.split(':')[0]) * 60 + parseInt(t.split(':')[1])
    );
    const [avgBed, avgWake] = [bedtimes, wakeTimes].map(
      (arr) => arr.reduce((a, b) => a + b, 0) / arr.length
    );
    const variance =
      bedtimes.reduce((s, t) => s + Math.abs(t - schedBed), 0) /
        bedtimes.length +
      wakeTimes.reduce((s, t) => s + Math.abs(t - schedWake), 0) /
        wakeTimes.length;
    return {
      avgBedtime: avgBed,
      avgWakeTime: avgWake,
      scheduledBedtime: schedBed,
      scheduledWakeTime: schedWake,
      consistencyScore: Math.max(0, Math.round(100 - variance / 2)),
  ...
  const getScheduleRecommendations = useCallback(() => {
    ...
    if (Math.abs(avgBedtime - scheduledBedtime) > 30) {
      const time = `${Math.floor(avgBedtime / 60)}:${String(
        Math.round(avgBedtime % 60)
      ).padStart(2, '0')}`;
      recs.push(
        `Consider adjusting bedtime to ${time} based on your actual patterns`
      );
    }
    if (Math.abs(avgWakeTime - scheduledWakeTime) > 30) {
      const time = `${Math.floor(avgWakeTime / 60)}:${String(
        Math.round(avgWakeTime % 60)
      ).padStart(2, '0')}`;
      recs.push(
        `Consider adjusting wake time to ${time} based on your actual patterns`
      );
    }
    if (consistencyScore > 80)
      recs.push("Great job! You're maintaining a consistent sleep schedule");
    return recs;
  }, [scheduleAnalysis]);
  ...
  const NumberRotator = ({ value, onChange, min, max, step = 1 }) => (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
      <Pressable
        onPress={() =>
          onChange(value <= min ? max : Math.max(min, value - step))
        }
        style={styles.button}
      >
      ...
      <Pressable
        onPress={() =>
          onChange(value >= max ? min : Math.min(max, value + step))
        }
        style={styles.button}
      >
        <Text style={styles.buttonText}>+</Text>
    ...
    return (
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
        <NumberRotator
          ...
          onChange={(nextH) =>
            onChange(
              `${String(nextH).padStart(2, '0')}:${String(m).padStart(2, '0')}`
            )
          }
          min={0}
          max={23}
        ...
        <NumberRotator
          ...
          onChange={(nextM) =>
            onChange(
              `${String(h).padStart(2, '0')}:${String(nextM).padStart(2, '0')}`
            )
          }
          min={0}
          max={55}
  ...
  const totalMinutesForDateKey = (dateKey) =>
    Math.round((sleepByDay[dateKey] ?? 0) * 60);
  ...
  const StackedWeekChart = ({
    ...
    return (
      <Animated.View
        style={{ width, height, transform: [{ translateX: animatedTranslateX }] }}
        {...panHandlers}
      >
        <View
          style={{
        ...
        >
          {['8h', '4h', '0h'].map((label, i) => (
            <Text
              key={i}
              style={{ color: '#5b6a82', fontSize: 10 }}
            >
              {label}
            </Text>
          ))}
        ...
        >
          {values.map((mins, i) => {
            ...
            const colors =
              selectedIndex === i
                ? ['#4e97ff', '#86adff', '#3a78ff', '#b9cfff']
                : ['#6ea8ff', '#9bb7ff', '#5b8bff', '#c6d8ff'];
            return (
              <Pressable
                key={i}
                onPress={() => onSelect?.(i)}
                style={{
                  ...
                  borderColor:
                    selectedIndex === i ? '#6ea8ff' : 'transparent',
                  borderRadius: 6,
              ...
              >
                ...
                />
                <View
                  style={{ height: coreH, backgroundColor: colors[1] }}
                />
                <View
                  style={{ height: deepH, backgroundColor: colors[2] }}
                />
                <View
                  style={{
                    height: remH,
  ...
  const SelectedWeekDayCard = () => {
    ...
    return (
      <View style={styles.card}>
        ...
        <Text
          style={{ color: theme.icon, marginBottom: 8 }}
        >
          Total:{' '}
          ...
          sessionsForDay.map((s) => (
            ...
            >
              <Text style={{ color: theme.text }}>
                {new Date(s.startISO).toLocaleTimeString([], {
                  hour: '2-digit',
                  minute: '2-digit',
                })}{' '}
                →{' '}
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        >
          ...
          <View style={{ alignItems: 'center' }}>
            ...
            >
              {Math.floor(avgAnimated)}h{' '}
              {String(Math.round((avgAnimated % 1) * 60)).padStart(
                2,
                '0'
              )}
              m
        ...
        >
          ...
          >
            <Text
              style={{ color: theme.tint, fontSize: 16 }}
            >
              {'<'} Prev
            </Text>
          </Pressable>
        ...
        <View style={styles.card}>
          ...
          <StackedWeekChart
            ...
            animatedTranslateX={weekTranslateX}
          />
        </View>
        ...
        <View style={styles.card}>
          ...
          >
            ...
            <TimeRotator
              ...
              onChange={(v) => saveSchedule({ bedtime: v })}
            />
          </View>
          <View
            style={{
          ...
          >
            ...
            <TimeRotator
              ...
              onChange={(v) => saveSchedule({ wakeTime: v })}
            />
          </View>
            ...
            <>
              ...
              >
                ...
                >
                  ...
                  <Text
                    style={{
                      color:
                        scheduleAnalysis.consistencyScore >= 70
                          ? '#4CAF50'
                          : scheduleAnalysis.consistencyScore >= 50
                          ? '#FF9800'
                          : '#F44336',
                      fontSize: 18,
                      fontWeight: '700',
                ...
                >
                  <View
                    style={{
                      height: '100%',
                      width: `${scheduleAnalysis.consistencyScore}%`,
                      backgroundColor:
                        scheduleAnalysis.consistencyScore >= 70
                          ? '#4CAF50'
                          : scheduleAnalysis.consistencyScore >= 50
                ...
                >
                  Based on last {scheduleAnalysis.totalSessions} sleep
                  sessions
                </Text>
              </View>
              ...
              >
                ...
                >
                  ...
                  >
                    {Math.floor(scheduleAnalysis.avgBedtime / 60)}:
                    {String(
                      Math.round(scheduleAnalysis.avgBedtime % 60)
                    ).padStart(2, '0')}
                  </Text>
                  <Text
                    style={{
                ...
                >
                  ...
                  >
                    {Math.floor(scheduleAnalysis.avgWakeTime / 60)}:
                    {String(
                      Math.round(scheduleAnalysis.avgWakeTime % 60)
                    ).padStart(2, '0')}
                  </Text>
                  <Text
                    style={{
                  ...
                  >
                    vs {schedule.wakeTime} scheduled
                  </Text>
                </View>
              </View>
              ...
              {getScheduleRecommendations().length > 0 && (
                ...
                >
                  ...
                  {getScheduleRecommendations().map(
                    (rec, index) => (
                      <Text
                        key={index}
                        style={{
          ...
          {(!scheduleAnalysis || scheduleAnalysis.totalSessions === 0) && (
            ...
            >
              Log some sleep sessions to see schedule analysis and
              recommendations
            </Text>
          )}
        </View>
        ...
        >
          ...
          <Text style={{ color: theme.icon, lineHeight: 22 }}>
            ...
            isn't just rest — it's your body's nightly reset. During
            deep sleep, your brain clears waste, rebuilds memory
            connections, and restores energy.{'\n\n'}
            🕰️ Keeping a{' '}
            <Text
              style={{
                fontWeight: '600',
            ...
            and wake-up time helps balance your internal clock,
            improving focus, energy, and mood.{'\n\n'}
            🌤️ Even small habits — like dimming lights, avoiding
            screens, or stretching — can help you fall asleep faster
            ...
            below to track your patterns and build better nights, one
            dream at a time.
          </Text>
        </View>
      </ScrollView>
